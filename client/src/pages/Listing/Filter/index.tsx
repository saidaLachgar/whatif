...
import { TPostHashtags, TPostSort } from 'src/model/post';
...
const Filter = (): JSX.Element | null => {
  ...
  const handleFilter = useCallback((id: string) => {
    ...
    navigate(`/${sort ?? TPostSort.HOT}/${newHashtag}`);
  }, [navigate, sort]);

  const handleSort = useCallback((sortBy: TPostSort) => {
    navigate(`/${sortBy}/${hashtag ?? ''}`);
  }, [navigate, hashtag]);
  ...
  return (
    <div className="Filter">
      {/* <ul className="Filter__apps">
        <li className="Filter__apps__item active">
          <img src="https://bytescale.mobbin.com/FW25bBB/image/content/app_logos/85e41e34-dcff-43a3-bf40-5ed2c83da1de.webp?f=webp&w=1920&q=85&fit=shrink-cover" alt="" />
          <span>Airbnb</span>
        </li>
        <li className="Filter__apps__item">
          <img src="https://bytescale.mobbin.com/FW25bBB/image/content/app_logos/21ae7ce6-23b8-46f5-af09-b35f73ba62c8.png?f=webp&w=1920&q=85&fit=shrink-cover" alt="" />
          <span>Airbnb</span>
        </li>
      </ul> */}
      <ul className="Filter__sort">
        <li
          onClick={() => { handleSort(TPostSort.HOT) }}
          className={classnames('Filter__sort__item', {
            'Filter__sort__item--active': !sort || sort === TPostSort.HOT,
          })}
        >
        ...
        <li
          onClick={() => { handleSort(TPostSort.NEW) }}
          className={classnames('Filter__sort__item', {
            'Filter__sort__item--active': sort === TPostSort.NEW,
          })}
        >
        ...
        <li
          onClick={() => { handleSort(TPostSort.TOP) }}
          className={classnames('Filter__sort__item', {
            'Filter__sort__item--active': sort === TPostSort.TOP,
          })}
        >
      ...
      {!!(data as TPostHashtags[])?.length &&
        <ul className="Filter__hashtags">
          ...
          >All</li>
          {(data as TPostHashtags[]).map(({ _id, count }, index) => (
            <li
              key={`hashtags-${index}`}
